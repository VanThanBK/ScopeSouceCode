...
namespace Scope
{
    ...
    public partial class MainForm : MetroForm
    {
        ...
        private void CalculateFreqAndRMS()
        {
            ...
            try
            {
                ...
                if(ToggleA.Checked == true)
                {
                    ...
                    if (EndA - BeginA > 3)
                    {
                        inpA = new Complex[1+EndA - BeginA];
                        for (int da = 0; da <= EndA - BeginA; da++)
                        {
                            inpA[da] = AVoltage[da + BeginA];
                        }
                        ka = true;
                        FFT(inpA);
                    }    else
                ...
                if (ToggleB.Checked == true)
                {
                    ...
                    if (EndB - BeginB > 3)
                    {
                        inpB = new Complex[1+EndB - BeginB];
                        for (int da = 0; da <= EndB - BeginB; da++)
                        {
                            inpB[da] = BVoltage[da + BeginB];
                        }
                        kb = true;
                        FFT(inpB);
                    } else
        ...
        private void DrawScope()
        {
            ...
            if (TimerValue > 0)
            {
                ...
                if (Kaaaaa == true)
                {
                    ...
                    YAxis.Title = "Biên Độ (V)";
                    XAxis.Minimum = 0;
                    XAxis.Maximum = Fzmax;
                    XAxis.MajorStep = Fzmax / 12;
                    XAxis.Title = "Tầng Số (Hz)";
        ...
        private static string _time_formatter1(double d)
        {
            if (d < 1E3)
            {
                return String.Format("{0:0.0}Hz", d);
            }
            else if (d >= 1E3 && d < 1E6)
            {
                return String.Format("{0:0.0}Hz", d / 1E3);
            }
            else if (d >= 1E6 && d < 1E9)
            {
                return String.Format("{0:0.0}Hz", d / 1E6);
            }
            else if (d >= 1E9)
            {
                return String.Format("{0:0.0}Hz", d / 1E9);
            }
            else
            {
                return String.Format("{0:0.0}Hz", d);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                Kaaaaa = false;
                button1.Text = "Phân Tích Phổ";

            } else
            {
                Kaaaaa = true;
                button1.Text = "Dạng Sóng";
            }
            DrawScope();
        }
